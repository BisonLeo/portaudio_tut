...
{	static RESAMPLE_PROG resample_progs [] =
	{	{	"sndfile-resample",
			...
			"libsamplerate",
			"sndfile-resample.exe --max-speed -c 0 -to %d source.wav destination.wav",
...
{	static SNR_TEST snr_test [] =
	{
		...
		{	2,	{ 0.311111, 0.49 },		78199,		2,	1.0 },  // worst case when generating 0.3111 and 0.49
...
{	double	snr, bandwidth, conversion_rate ;
	...
	conversion_rate = (1.0 * output_samples * freq.QuadPart); // / time_data.tms_cutime;
